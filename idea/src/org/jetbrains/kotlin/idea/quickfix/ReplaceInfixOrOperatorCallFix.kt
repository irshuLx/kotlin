    ...
    override fun invoke(project: Project, editor: Editor?, file: KtFile) {
        ...
        when (element) {
            is KtArrayAccessExpression -> {
                ...
                if (assignment != null && right != null) {
                    val newExpression = psiFactory.createExpressionByPattern(
                            "$0?.set($1, $2)", element.arrayExpression!!, element.indexExpressions.joinToString(", ") { it.text }, right)
                    assignment.replace(newExpression)
                }
                else {
                    val newExpression = psiFactory.createExpressionByPattern(
                            "$0?.get($1)", element.arrayExpression!!, element.indexExpressions.joinToString(", ") { it.text })
                    element.replace(newExpression)
                }
            }
            is KtCallExpression -> {
                val newExpression = psiFactory.createExpressionByPattern(
                        "$0?.invoke($1)", element.calleeExpression!!, element.valueArguments.joinToString(", ") { it.text })
                element.replace(newExpression)
            }
